...
function SignUp() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await axios.post('http://localhost:8000/api/v1/user/signup/', userData);
      console.log('Регистрация успешна:', response.data);
      navigate('/');
    } catch (error) {
      console.error('Ошибка при регистрации:', error.response?.data);
      alert('Ошибка при регистрации! Пожалуйста, проверьте ваши данные и попробуйте снова.');
    }
  };

  const handleGoogleRegister = async () => {
    try {
      // Ваш код для регистрации через Google
    ...
    <section className="m-8 flex">
      ...
      <div className="w-full lg:w-3/5 flex flex-col items-center justify-center">
        <div className="text-center">
          <Typography variant="h2" className="font-bold mb-4">Присоединяйтесь к нам сегодня</Typography>
        ...
        <form className="mt-8 mb-2 mx-auto w-80 max-w-screen-lg lg:w-1/2" onSubmit={handleSubmit}>
          ...
          <Typography variant="paragraph" className="text-center text-blue-gray-500 font-medium mt-4">
            Уже есть аккаунт?
            <Link to="/sign-in" className="text-gray-900 ml-1">Войти</Link>
          </Typography>
        </form>
        <Button
          size="lg"
